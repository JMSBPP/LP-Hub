...
describe('TickMath', () => {
    ...
    describe('#getSqrtRatioAtTick', () => {
        it('throws for too low', async () => {
            await (0, expect_1.expect)(tickMath.getSqrtRatioAtTick(MIN_TICK - 1)).to.be.revertedWithCustomError(tickMath, 'tickOutOfRange');
        });
        it('throws for too low', async () => {
            await (0, expect_1.expect)(tickMath.getSqrtRatioAtTick(MAX_TICK + 1)).to.be.revertedWithCustomError(tickMath, 'tickOutOfRange');
        });
        it('min tick', async () => {
        ...
        it('min tick ratio is less than js implementation', async () => {
            (0, expect_1.expect)(await tickMath.getSqrtRatioAtTick(MIN_TICK)).to.be.lt((0, utilities_1.encodePriceSqrt)(1, 2n ** (127n)));
        });
        it('max tick ratio is greater than js implementation', async () => {
            (0, expect_1.expect)(await tickMath.getSqrtRatioAtTick(MAX_TICK)).to.be.gt((0, utilities_1.encodePriceSqrt)(2n ** (127n), 1));
        });
        it('max tick', async () => {
    ...
    describe('#getTickAtSqrtRatio', () => {
        it('throws for too low', async () => {
            await (0, expect_1.expect)(tickMath.getTickAtSqrtRatio(utilities_1.MIN_SQRT_RATIO - 1n)).to.be.revertedWithCustomError(tickMath, 'priceOutOfRange');
        });
        it('throws for too high', async () => {
            await (0, expect_1.expect)(tickMath.getTickAtSqrtRatio(utilities_1.MAX_SQRT_RATIO)).to.be.revertedWithCustomError(tickMath, 'priceOutOfRange');
        });
        ...
        it('ratio closest to max tick', async () => {
            (0, expect_1.expect)(await tickMath.getTickAtSqrtRatio(utilities_1.MAX_SQRT_RATIO - 1n)).to.eq(MAX_TICK - 1);
        });
        for (const ratio of [
            utilities_1.MIN_SQRT_RATIO,
            (0, utilities_1.encodePriceSqrt)(10n ** (12n), 1),
            (0, utilities_1.encodePriceSqrt)(10n ** (6n), 1),
            ...
            (0, utilities_1.encodePriceSqrt)(1, 10n ** (6n)),
            (0, utilities_1.encodePriceSqrt)(1, 10n ** (12n)),
            utilities_1.MAX_SQRT_RATIO - 1n,
        ]) {
            describe(`ratio ${ratio}`, () => {
                ...
                it('result', async () => {
                    (0, expect_1.expect)(Number(await tickMath.getTickAtSqrtRatio(ratio))).to.matchSnapshot();
                });
                it('gas [ @skip-on-coverage ]', async () => {
